...
namespace FlashCards.DAL.Commands
{
    ...
    static class SetOfFlipCards
    {
        ...
        public static bool EditFlipCard(FlipCard fpc, uint idFpc, sbyte idDeck)
        {
            ...
            using (var connection = DBConnection.Instance.Connection)
            {
                string CHANGE_FLIPCARD = $"UPDATE flipcard SET FrontContent=@frontC, BackContent=@backC WHERE ID=@idF AND ID_Deck=@idD";
                MySqlCommand cmd = new MySqlCommand(CHANGE_FLIPCARD, connection);
